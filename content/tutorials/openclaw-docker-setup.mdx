...
howTo:
  - name: "Install Docker"
    text: "Install Docker Desktop on macOS or Windows, or Docker Engine 24+ on Linux. Verify with `docker --version` and `docker compose version`."
  - name: "Pull the OpenClaw image"
    text: "Run `docker pull ghcr.io/openclaw/openclaw:latest` to fetch the official multi-arch image (~800MB)."
  - name: "Create a docker-compose.yml"
    text: "Define the openclaw service with port 3000:3000, volumes for ~/.openclaw and your workspace, and your LLM provider API keys as environment variables."
  - name: "Start the container"
    text: "Run `docker compose up -d` to start OpenClaw in the background. Confirm with `docker compose ps`."
  - name: "Open the OpenClaw UI"
    text: "Visit http://localhost:3000 to complete the onboarding wizard, configure your provider, and install your first skills."
faqs:
  - question: "Can OpenClaw run on Docker?"
    answer: "Yes. OpenClaw publishes an official Docker image at ghcr.io/openclaw/openclaw and supports Docker Compose for production deployments. Docker is the recommended way to run OpenClaw on a server, and works for local development on Mac, Windows, and Linux."
  - question: "Is running OpenClaw on Docker safe?"
    answer: "Running OpenClaw in Docker is generally safer than native installation because the agent operates inside a container, isolated from your host filesystem. For best security, mount only the directories OpenClaw needs, run with a non-root user, and use Docker secrets for API keys instead of plain environment variables."
  - question: "How do I get the OpenClaw Docker token?"
    answer: "The OpenClaw Docker image itself is public — no token required to pull. If you mean an OpenClaw API token for programmatic access, run `docker exec openclaw claw token create --name my-token` once the container is running."
